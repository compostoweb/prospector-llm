...
export default function CadenciasPage() {
  ...
  function handleToggle(id: string, current: boolean) {
    const nextIsActive = !current

    toggleCadence.mutate(
      { id, is_active: nextIsActive },
      {
        onSuccess: () => {
          toast.success(nextIsActive ? "Cadência ativada" : "Cadência desativada")
        },
        onError: (error) => {
          toast.error(
            error instanceof Error ? error.message : "Falha ao alterar o status da cadência",
          )
        },
      },
    )
  ...
  function handleDeleteRequest(cadence: Cadence) {
    setCadenceToDelete(cadence)
  }
  ...
  function handleDeleteConfirm() {
    ...
    deleteCadence.mutate(cadenceToDelete.id, {
      onSuccess: () => {
        setCadenceToDelete(null)
        toast.success("Cadência excluída")
      },
      onError: (error) => {
        toast.error(error instanceof Error ? error.message : "Falha ao excluir cadência")
      },
    })
  }
  ...
  return (
    <>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <KPICard
            ...
            sub={
              totalLeads > 0
                ? `${Math.round((totalActive / totalLeads) * 100)}% do total`
                : undefined
            }
          />
          <KPICard
      ...
      <AlertDialog
        open={cadenceToDelete !== null}
        onOpenChange={(open) => !open && setCadenceToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {cadenceToDelete
                ? `A cadência "${cadenceToDelete.name}" será removida da listagem e seus steps relacionados serão apagados.`
                : "Confirme a exclusão desta cadência."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleteCadence.isPending}>Cancelar</AlertDialogCancel>
            <AlertDialogAction
              onClick={handleDeleteConfirm}
              disabled={deleteCadence.isPending}
              className="bg-(--danger) text-white hover:bg-(--danger) hover:opacity-90"
            >
              {deleteCadence.isPending ? "Excluindo..." : "Excluir cadência"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
