...
Responsabilidades:
  ...
  3. Injetar rodapé de conformidade no HTML do e-mail antes do envio
...
def inject_tracking(
    ...
    Injeta no corpo HTML:
      1. Pixel de rastreamento de abertura (1×1 GIF invisível)
      2. Link de unsubscribe no rodapé

    Retorna o HTML modificado.
    """
    pixel_url = build_open_pixel_url(interaction_id)
    unsub_url = build_unsubscribe_url(tenant_id, email)
...
<div style="margin-top:32px;padding-top:16px;border-top:1px solid #e5e7eb;
            font-family:Arial,sans-serif;font-size:11px;color:#9ca3af;text-align:center;">
  <p style="margin:0;">
    Você está recebendo este e-mail pois seu endereço faz parte de nossa lista de prospecção.
    <br />
    <a href="{unsub_url}" style="color:#6b7280;text-decoration:underline;">
      Clique aqui para não receber mais e-mails
    </a>
  </p>
</div>
{pixel_tag}
